...
# Write the Log.
def write_it(rows, outfile=None, quiet=True):
    ...
    try:
        ...
        if not outfile:
            ...
            if not quiet: print "[+] Writing output to %s..."%outfile
            try:
                if "StringIO" in outfile:
                    return_rows = []
                    for row in rows:
                        return_rows.append(",".join(["%s" % x for x in row]))
                    return return_rows
                else:
                    f = open(outfile, 'wb')
                    if g_usebom:
...
def validateSystemFile(fileFullPath):
    ...
    try:
        reg = Registry.Registry(fileFullPath)
    except Registry.RegistryParse.ParseException, err:
        print "[-] Error parsing %s: %s" % (fileFullPath, err)
        return False
    ...
    try:
        tmp = reg.root().name()
        session_man_key = reg.open('ControlSet001\\Control')
        # session_man_key = reg.open('%s\\ControlSet001\\Control\\Session Manager' % reg.root().name())
        return True
    except Registry.RegistryKeyNotFoundException:
        return False
...
def read_from_hive(hive, quiet=False):
    out_list = []
    tmp_list = []
